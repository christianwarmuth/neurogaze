...
def get_callback_for_file(filename):
    ...
    windows_apps = create_encoding_dict(f'{filename}.apps.json')
    windows_tabs = create_encoding_dict(f'{filename}.tabs.json')
    last_windows_apps_size = 0
    last_windows_tabs_size = 0

    with open(filename, 'w') as f, \
            open(f'{filename}.apps.json', 'w') as apps, \
            open(f'{filename}.tabs.json', 'w') as tabs:
        ...
        def callback(
            ...
            # global last_windows_apps_size
            # global last_windows_tabs_size
            ...
            short_name_id = windows_apps[short_name]
            title_id = windows_tabs[title.value]
            ...
            if last_windows_apps_size < len(windows_apps):
                apps.seek(0)
                json.dump(windows_apps, apps)
                apps.truncate()

                apps.flush()

            if last_windows_tabs_size < len(windows_tabs):
                tabs.seek(0)
                json.dump(windows_tabs, tabs)
                tabs.truncate()

                tabs.flush()

            last_windows_apps_size = len(windows_apps)
            last_windows_tabs_size = len(windows_tabs)
